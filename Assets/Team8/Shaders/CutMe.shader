...
Shader "Team8/CutMe"
{
	...
	_Darken("Darken",Range(1,10)) = 1
	_MainTex("Base (RGB) Trans (A)", 2D) = "white" {}
   _WorldY("Clip Below World Y", Float) = 2.5
	   _Steps("CuttingWorldPosition",Vector) = (0,0,0,0)
	   _Axis("CuttingWorldAxis",Vector) = (0,1,0)
	}
		...
		SubShader{
			...
			Pass {
				...
				fixed4 frag(v2f i) : COLOR {
					...
					c.a = 1;
					c /= _Darken;
					float h = _WorldY - (i.worldPos * _Axis + i.worldPos.y * _Axis);
					clip(h);
					return c;
				}
